...
has('type AppPage = "home" | "editor" | "audio" | "output" | "server" | "devices" | "settings";', "distinct AppPage type");
has('useState<AppPage>("home")', "Home default page");
for (const page of ["home", "editor", "audio", "output", "server", "devices", "settings"]) has(`["${page}"`, `${page} navigation entry`);
...
console.log("[ui-nav-audit] PASS — Home, Editor, Audio, Output, Server, Devices, and Settings are distinct pages with dedicated control groups.");
