...
renderer_path = Path(__file__).resolve().parents[1] / "src" / "render" / "renderer.ts"
src = renderer_path.read_text(encoding="utf-8")
pattern = re.compile(r'  } else if \(k < 31\.5\) \{\n.*?\n  } else if \(k < 32\.5\) \{', re.S)
replacement = r'''  } else if (k < 31.5) {
...
next_src, count = pattern.subn(replacement, src, count=1)
if count != 1:
    raise SystemExit(f"Expected exactly one RealisticFlame block, replaced {count}")
renderer_path.write_text(next_src, encoding="utf-8")

types_path = Path(__file__).resolve().parents[1] / "src" / "scene" / "types.ts"
types = types_path.read_text(encoding="utf-8")
old = '''    speed: 1, scale: 1, audio: "Manual", audioInfluence: 0.7, color: "#f4d27a", color2: "#7ad0ff",
    color3: "#ffffff", p0: 0.65, p1: 0.5, p2: 0.4, preset: "Default",'''
new = '''    speed: 1, scale: 1, audio: "Manual", audioInfluence: 0.7,
    ...
    preset: "Default",'''
if old not in types:
    raise SystemExit("RealisticFlame defaults anchor not found")
types_path.write_text(types.replace(old, new, 1), encoding="utf-8")

print("RealisticFlame reference pass applied")
